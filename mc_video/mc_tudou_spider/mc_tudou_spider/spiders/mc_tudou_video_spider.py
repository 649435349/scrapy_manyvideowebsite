...
class mc_tudou_spider(scrapy.Spider):
    ...
    def start_requests(self):
        if self.choice == 'create':
            ...
            sql = 'select distinct page_url,upload_time from src_minecraft_public_video_day where website=%s and play_cnt>1000 and upload_time>%s'
            self.cur.execute(sql, ['tudou', d])
    ...
    def get_each_video(self,response):
        for i in response.xpath('//div[@class="v"]'):
            ...
            upload_time=self.get_now(i.xpath('.//span[@class="r"]').extract()[0]).split()[0]
