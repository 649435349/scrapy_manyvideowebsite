    ...
    def get_author_information(self, response):
        ...
        try:
            publicItem['post_cnt'] = self.nowanyi(
                response.xpath('//ul[@class="user_count"]/li[3]/span[2]/text()').extract()[0])
            publicItem['all_play_cnt'] = self.nowanyi(
                response.xpath('//ul[@class="user_count"]/li[7]/span[2]/text()').extract()[0])
            yield publicItem
        except:
            # 老版的腾讯视频
            publicItem['post_cnt'] = self.nowanyi(response.xpath(
                '//li[@class="user_count_video"]/span[@class="count_num"]/text()').extract()[0])
            publicItem['all_play_cnt'] = self.nowanyi(response.xpath(
                '//li[@class="user_count_play"]/span[@class="count_num"]/text()').extract()[0])
            yield publicItem
        for i in range(1, 11):
