...
class mc_acfun_spider(scrapy.Spider):
    ...
    def start_requests(self):
        if self.choice == 'create':
            ...
            self.cur.execute(sql,('acfun'))
            self.gets=self.cur.fetchall()
            for i in self.gets():
                acid=re.findall(r'(?<=/ac)\d+',i[0])[0]
                fenp=True if '_' in i[0] else False
                yield Request(i[0], callback=self.get_video_information,meta={'fenp':fenp,'acid':acid})
        else:
            print 'sth wrong.'
    ...
    def get_video_information(self,response):
        try:
            ...
            publicItem['upload_time'] =item['upload_time']=d['contributeTime']
            item['personal_homepage']='http://www.acfun.cn/u/{}.aspx#page=1'.format(d['userId'])
