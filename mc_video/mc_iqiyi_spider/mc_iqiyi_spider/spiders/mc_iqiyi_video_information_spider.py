    ...
    def get_author_information(self, response):
        ...
        try:
            publicItem['fans_cnt'] = response.xpath(
                '//div[@class="info_connect"]').xpath('.//em[@class="count"]/a/@data-countnum').extract()[0].replace(',', '')
            publicItem['all_play_cnt'] = self.nowanyi(response.xpath(
                '//div[@class="info_connect"]').xpath('.//em[@class="count"]/a/text()').extract()[0].replace(',', ''))
