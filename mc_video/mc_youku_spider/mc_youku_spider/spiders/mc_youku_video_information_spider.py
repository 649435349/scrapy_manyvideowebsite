...
class mc_youku_spider(scrapy.Spider):
    ...
    def start_requests(self):
        if self.choice=='create':
            ...
            sql = 'select distinct page_url,duration,author,upload_time from src_minecraft_youku_video_day where play_cnt>1000 and upload_time>%s'
    ...
    def get_each_video(self, response):
        for i in response.xpath('//div[@class="v"]'):
            ...
            item['upload_time'] = self.get_now(i.xpath(
                './/div[@class="v-meta-entry"]/div[last()]/span[last()]/text()').extract()[0]).split()[0]
            yield Request(item['page_url'], callback=self.get_video_information, meta={'item': item, 'id': idd})
        ...
        try:
            ...
            try:
                item['category'] = response.xpath(
                    '//h1[@class="title"]/a[1]/text()').extract()[0].strip('[').strip(']')
            except:
                # 有的视频没有分类
                item['category'] = ''
    ...
    def get_json1(self, response):
        ...
        item['play_cnt']=d['data']['stat']['vv'].replace(',','')
    ...
    def get_json2(self, response):
        ...
        item['personal_channel'] ='http://i' + d['data']['channelurl'][3:]
    ...
    def get_author_video(self,response):
        for div in response.xpath('//div[@class="items"]/div[@class="v va"]'):
            ...
            item['upload_time'] = div.xpath('.//span[@class="v-publishtime"]/text()').extract()[0].split()[0]
